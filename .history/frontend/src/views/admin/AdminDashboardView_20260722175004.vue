...
const occupiedSpacesCount = computed(() => {
  return statistics.value.occupiedSpaces ?? 0
})

const availableSpacesCount = computed(() => {
  return statistics.value.availableSpaces ?? 0
})

const occupancyPercentage = computed(() => {
  const totalSpaces = spaceOccupancy.value.length
  ...
  return Math.round(
    (occupiedSpacesCount.value / totalSpaces) * 100,
  )
})
...
async function loadDashboard() {
  ...
  try {
    ...
    statistics.value = {
      ...
      todayReservations:
        data.statistics?.todayReservations ?? 0,

      activeSpaces:
        data.statistics?.activeSpaces ?? 0,

      monthlyRevenue:
        data.statistics?.monthlyRevenue ?? 0,

      monthlyReservations:
        data.statistics?.monthlyReservations ?? 0,

      occupiedSpaces:
        data.statistics?.occupiedSpaces ?? 0,

      availableSpaces:
        data.statistics?.availableSpaces ?? 0,
    }

    todayReservations.value = Array.isArray(
      data.todayReservations,
    )
      ? data.todayReservations
      : []

    recentReservations.value = Array.isArray(
      data.recentReservations,
    )
      ? data.recentReservations
      : []

    spaceOccupancy.value = Array.isArray(
      data.spaceOccupancy,
    )
      ? data.spaceOccupancy
      : []
    ...
    console.error(
      'Greška prilikom dohvaćanja dashboarda:',
      error,
    )
    ...
    if (error.response?.status === 403) {
      errorMessage.value =
        'Nemate dopuštenje za pristup dashboardu.'
    ...
    errorMessage.value =
      ...
      'Nije moguće učitati podatke dashboarda.'
  } finally {
...
<template>
  <section class="dashboard-page">
    ...
    >
      <div>
        <strong>Došlo je do pogreške</strong>
    ...
    >
      ...
      <p>Učitavanje podataka dashboarda...</p>
    </div>
        ...
        <article class="stat-card">
          <div class="stat-card__top">
            ...
            <span class="stat-badge stat-badge--positive">
              {{ statistics.monthlyReservations }}
              ovaj mjesec
            </span>
          </div>
