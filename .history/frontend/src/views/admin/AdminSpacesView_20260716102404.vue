...
const subtypeOptions = computed(() => {
  ...
  return options[createForm.value.space_type] || []
})

function handleSpaceTypeChange() {
  createForm.value.space_subtype = ''

  if (createForm.value.space_type !== 'conference') {
    createForm.value.is_modular = false
    createForm.value.combination_group = null
  }
}
...
function addEquipment() {
  ...
  const alreadyExists = createForm.value.equipment.some(
    (item) =>
  ...
  createForm.value.equipment.push({ name })
  newEquipmentName.value = ''
}

function removeEquipment(index) {
  createForm.value.equipment.splice(index, 1)
}
...
function handleFilesSelected(event) {
  ...
  console.log('Odabrane slike:', selectedFiles.value)
}
...
async function submitCreateSpace() {
  ...
  try {
    const payload = {
      ...
      name_en:
        createForm.value.name_en.trim() || null,

      description_en:
        createForm.value.description_en.trim() || null,

      space_type: createForm.value.space_type,
      space_subtype: createForm.value.space_subtype,

      capacity: Number(createForm.value.capacity),
      price: Number(createForm.value.price),
      price_unit: createForm.value.price_unit,

      is_modular:
        createForm.value.space_type === 'conference'
          ? createForm.value.is_modular
          : false,
      ...
      equipment: createForm.value.equipment.map(
        (item) => ({
          name: item.name,
        }),
      ),
