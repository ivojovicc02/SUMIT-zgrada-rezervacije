...
def get_dashboard(
    ...
    # Današnje aktivne rezervacije
    today_reservations_query = (
    ...
    today_reservation_rows = (
        today_reservations_query.all()
    )

    # Za sada se svi postojeći prostori smatraju aktivnima.
    # Ako Space model ima is_active, dodaj filter.
    active_spaces = (
    ...
    # Broj rezervacija kreiranih ili održanih ovaj mjesec.
    # Ovdje se broje prema terminu održavanja.
    ...
    # Posljednjih pet rezervacija
    recent_reservation_rows = (
        db.query(Reservation, Space)
        .join(
            Space,
            Space.id == Reservation.space_id,
    ...
    for reservation, space in today_reservation_rows:
        customer_name = (
            reservation.company.strip()
    ...
    for reservation, space in recent_reservation_rows:
        customer_name = (
            reservation.company.strip()
            if reservation.company
            and reservation.company.strip()
    ...
    for space in spaces:
        ...
        if current_reservation:
            space_occupancy.append({
                ...
                "status": "occupied",
                "currentEvent": (
                    current_reservation.event_name
                ),
                "occupiedUntil": (
    ...
    occupied_spaces_count = sum(
        1
        for space in space_occupancy
        if space["status"] == "occupied"
    )

    available_spaces_count = sum(
        1
        for space in space_occupancy
